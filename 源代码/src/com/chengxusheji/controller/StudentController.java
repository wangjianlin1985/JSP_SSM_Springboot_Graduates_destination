...
public class StudentController extends BaseController {
	...
	public void list(String studentNumber,String name,@ModelAttribute("gradeObj") GradeInfo gradeObj,@ModelAttribute("specialObj") SpecialInfo specialObj,String telephone,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (studentNumber == null) studentNumber = "";
		if (name == null) name = "";
		if (telephone == null) telephone = "";
		if(rows != 0)studentService.setRows(rows);
		List<Student> studentList = studentService.queryStudent(studentNumber, name, gradeObj, specialObj, telephone, page);
	    /*计算总的页数和总的记录数*/
	    studentService.queryTotalPageAndRecordNumber(studentNumber, name, gradeObj, specialObj, telephone);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String studentNumber,String name,@ModelAttribute("gradeObj") GradeInfo gradeObj,@ModelAttribute("specialObj") SpecialInfo specialObj,String telephone,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (studentNumber == null) studentNumber = "";
		if (name == null) name = "";
		if (telephone == null) telephone = "";
		List<Student> studentList = studentService.queryStudent(studentNumber, name, gradeObj, specialObj, telephone, currentPage);
	    /*计算总的页数和总的记录数*/
	    studentService.queryTotalPageAndRecordNumber(studentNumber, name, gradeObj, specialObj, telephone);
	    /*获取到总的页码数目*/
	    int totalPage = studentService.getTotalPage();
	...
	public void OutToExcel(String studentNumber,String name,@ModelAttribute("gradeObj") GradeInfo gradeObj,@ModelAttribute("specialObj") SpecialInfo specialObj,String telephone, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(studentNumber == null) studentNumber = "";
        if(name == null) name = "";
        if(telephone == null) telephone = "";
        List<Student> studentList = studentService.queryStudent(studentNumber,name,gradeObj,specialObj,telephone);
        ExportExcelUtil ex = new ExportExcelUtil();
        String title = "Student信息记录"; 
        String[] headers = { "学号","姓名","性别","学生照片","年级","专业","联系电话","邮箱"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<studentList.size();i++) {
        	Student student = studentList.get(i); 
        	dataset.add(new String[]{student.getStudentNumber(),student.getName(),student.getSex(),student.getStudentPhoto(),student.getGradeObj().getGradeName(),student.getSpecialObj().getSpecialName(),student.getTelephone(),student.getEmail()});
        }
        /*
